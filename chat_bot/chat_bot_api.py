...
from mongodb_connection import get_data, overwrite_data, human_ans_post, human_ans_get, predicted_ques_ans_post, predicted_ques_ans_get, app
from check_if_ques_and_save import is_question, write_sentences_to_json
...
@app.route("/get_predicted_ques_ans", methods=["GET"])
...
@app.route("/predict_ques_ans_and_post", methods=["GET"])
...
@app.route("/post_ques_ans", methods=["POST"])
def post_ques_ans():
    ques_ans = request.json
    if len(ques_ans) > 0:
...
@app.route("/get_ques_ans", methods=["GET"])
def get_ques_ans():
